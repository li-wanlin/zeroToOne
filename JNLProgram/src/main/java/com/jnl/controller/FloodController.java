...
/*        logger.info("这是防汛保障除文件以外的数据：{}", JSON.toJSONString(unit));
        if (files == null){
            logger.info("这次防汛保障未加入图片数据");
        }else {
            logger.info("这次防汛保障加入图片数量为：{}",files.length);
        }*/
