...
import com.stg.vo.functionVo.Base64Vo;
...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        //设置免token界面
/*        if (request.getRequestURI().endsWith("/STGProgram/loginByName")
                || request.getRequestURI().endsWith("/STGProgram/refresh")
                || request.getRequestURI().endsWith("/STGProgram/logout")){
            return true;
        }*/
        ...
        // 放行登录相关接口
        if (requestURI.endsWith("/STGProgram/loginByName")
                || requestURI.endsWith("/STGProgram/refresh")
                || requestURI.endsWith("/STGProgram/logout")) {
            return true;
        }

        // 放行静态资源目录
        if (requestURI.startsWith(request.getContextPath() + "/fg/")        // 匹配 /STGProgram/fg/
        ...
        // 放行特定文件
        if (requestURI.endsWith("/STGProgram/config.js")
                || requestURI.endsWith("/STGProgram/favicon.ico")
                || requestURI.endsWith("/STGProgram/index.html")) {
            return true;
        }
