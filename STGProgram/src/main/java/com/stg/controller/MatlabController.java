...
import com.stg.service.impl.AverageFlowServiceImpl;
import com.stg.service.impl.DamSignServiceImpl;
import com.stg.vo.damSignVo.SignResponse;
import com.stg.vo.damSignVo.SignUnit;
import com.stg.vo.functionVo.Meta;
import com.stg.vo.matlabVo.ResUnit;
import com.stg.vo.matlabVo.SkwResponse;
...
public class MatlabController {
    ...
    @GetMapping("/fourTotal/Matlab/selectAverageFlow")
    public SkwResponse selectAverageFlow(){
        SkwResponse response = new SkwResponse();
        Meta meta = new Meta();
        response.setMeta(meta);

        try {
            ResUnit unit = averageFlowService.selectAverageFlow();
            if (unit != null){
                response.setUnit(unit);
                meta.setStatus(200);
                meta.setMsg("获取小时平均来流量成功");
                return response;
            }

        }catch (Exception e){
            logger.error("获取小时平均来流量Controller层发生异常",e);
        }
        meta.setStatus(400);
        meta.setMsg("获取小时平均来流量失败");
        return response;
    }
