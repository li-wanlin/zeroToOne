...
import com.stg.service.impl.DisplayTextServiceImpl;
import com.stg.service.impl.SafetyServiceImpl;
import com.stg.utils.DateLocalUtils;
import com.stg.vo.StaticTextVo.DisplayRequest;
import com.stg.vo.StaticTextVo.DisplayResponse;
import com.stg.vo.StaticTextVo.SafetyRequest;
import com.stg.vo.StaticTextVo.SafetyResponse;
...
public class StaticTextController {
     ...
     * @return
     */
    @GetMapping("/homePage/display/selectAmountInfo")
    public DisplayResponse selectAmountInfo(){
        DisplayResponse displayResponse = new DisplayResponse();
        Meta meta = new Meta();
        meta.setStatus(400);
        meta.setMsg("获取水量信息数据失败");
        displayResponse.setMeta(meta);
        displayResponse.setUpdateTime(DateLocalUtils.parseTimeToStr(LocalDateTime.now()));

        try {
            List<Double> amountInfos = displayTextService.selectAmountInfo();
            if (amountInfos != null){
                meta.setStatus(200);
                meta.setMsg("获取水量信息数据成功");


                displayResponse.setAmountInfos(amountInfos);
                return displayResponse;
            }
        }catch (Exception e){
            logger.error("在获取水量信息Controller层发生异常",e);
        }
     ...
     * @return
     */
    @PostMapping("/homePage/display/updateByAi")
    public Meta updateByAi(@RequestBody DisplayRequest displayRequest){
        Meta meta = new Meta();
        try{
            List<Double> amountInfos = displayRequest.getAmountInfos();
            Boolean updateByAs = displayTextService.updateByAi(amountInfos);
            if (updateByAs){
                meta.setStatus(200);
                meta.setMsg("修改水量信息数据成功");
                return meta;
            }
        }catch (Exception e){
            logger.error("在修改水量信息Controller层发生异常",e);
        }

        meta.setStatus(400);
        meta.setMsg("修改水量信息数据失败");
        return meta;
    }
