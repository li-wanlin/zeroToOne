...
import com.stg.mapper.FloodControlMapper;
import com.stg.service.FloodControlService;
...
public class FloodControlServiceImpl extends ServiceImpl<FloodControlMapper, FloodControl> implements FloodControlService {
        ...
        try {
            ...
            if (maxFlow > 437.1){
                ...
                display = "未来72小时内，水库预计最大泄量"+displayFlow+"m3//s，最高水位"+displayStage+"m，出现时间为"+maxTime
                        +"，应启动Ⅰ级红色应急响应；预计下泄流量大于349m3//s的时段为"+startTime+"至"+endTime+"，共计"+(endIndex-startIndex)+"小时；";

                map.put("display",display);
                map.put("planLv","1");

            }else if (maxFlow > 181.2){
                ...
                display = "未来72小时内，水库预计最大泄量"+displayFlow+"m3//s，最高水位"+displayStage+"m，出现时间为"+maxTime
                        +"，应启动Ⅱ级橙色应急响应；预计下泄流量大于144m3//s的时段为"+startTime+"至"+endTime+"，共计"+(endIndex-startIndex)+"小时；";

                map.put("display",display);
                map.put("planLv","2");
            }else if (maxFlow > 141.8){
                for (int i = 0; i < flows.length; i++) {
                    if (flows[i] > 113){
                        startIndex = i;
                        break;
                ...
                display = "未来72小时内，水库预计最大泄量"+displayFlow+"m3//s，最高水位"+displayStage+"m，出现时间为"+maxTime
                        +"，应启动Ⅲ级黄色应急响应；预计下泄流量大于113m3//s的时段为"+startTime+"至"+endTime+"，共计"+(endIndex-startIndex)+"小时；";
    ...
    public Map<String, String> selectPlanSim() {
        try {
            ...
            String planLv = "2";
            LocalDateTime nowHour = LocalDateTime.now().withMinute(0).withSecond(0).withNano(0);
            String startHour = DateLocalUtils.parseTimeToStr(nowHour.plusHours(24));
            String maxHour = DateLocalUtils.parseTimeToStr(nowHour.plusHours(25));
            String endHour = DateLocalUtils.parseTimeToStr(nowHour.plusHours(29));

            String display = "未来72小时内，水库预计最大泄量341.47m³/s，最高水位942.90m，出现时间为"+maxHour
                    +"，应启动Ⅱ级橙色应急响应；预计下泄流量大于181.2m³/s的时段为"+startHour+"至"+endHour+"，共计5小时；";
            map.put("display",display);
