...
import com.stg.mapper.SafetyWarnMapper;
import com.stg.service.SafetyWarnService;
import com.stg.utils.DateLocalUtils;
import com.stg.vo.fourPreventVo.WarnResponse;
import com.stg.vo.fourPreventVo.WarnUnit;
...
public class SafetyWarnServiceImpl extends ServiceImpl<SafetyWarnMapper, SafetyWarn> implements SafetyWarnService {
    ...
    public void saveOnenet(SeepageInfo info) {
        try {
            ...
            if (lv3 != null && info.getLv3() > limit && DateLocalUtils.parseDateToLong(info.getUpdateTime()) > (deviceSevenThree+1000*60*10)){
                SafetyWarn safetyWarn = new SafetyWarn();
                safetyWarn.setUpdateTime(info.getUpdateTime());
                safetyWarn.setMonValue(info.getLv3());
                safetyWarn.setDevice("汇水沟");
                safetyWarn.setLimitType("渗流量");
                safetyWarn.setLimitState("+"+df.format(info.getLv3()-limit)+"L/s");
